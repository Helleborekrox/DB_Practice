  ...
  <body>
	...
	h1 {
		...
		<div class="container" >
			...
			<hr>
				<table class="table-condensed" data-id="user">
			  		<thead>
					    <tr>
					      <th scope="col"><strong>№</strong></th>
					      <th scope="col"><strong>Имя</strong></th>
					      <th scope="col"><strong>Пароль</strong></th>
					...
					<tbody id="content">
						<?php
		                  $query = "SELECT * FROM user JOIN images ON user.user_image=images.image_id"; //записываем запрос на выборку данных
		                  ...
		                  for($i = 0, $count = sizeof($data); $i < $count; $i++) // выводим данные в виде строк HTML-таблицы 
		                  {
		                  	echo "
					      		...
					      		<td><input class='form-control' name='name' disabled type='text' data-type='s' value='{$data[$i]['name']}'></td>

					      		<td><input class='form-control' name='password' disabled data-type='s' type='text' value='{$data[$i]['password']}' ></td>

					      		<td><img src='{$data[$i]['image']}' data-type='s' alt='{$data[$i]['image']}'></td>

					      		<td><input class='form-control' name='email' data-type='s' disabled type='text' value='{$data[$i]['email']}'></td>
					      		
					      		<td>
								<div class='action'>
									<i class='fa fa-pencil' aria-hidden='true'></i>
									<i class='fa fa-trash' data-table='user' data-id='{$data[$i]['id']}' aria-hidden='true'></i>
								</div>
								<div class='pencil'>
									<i class='fa fa-check' data-table='user' data-id='{$data[$i]['id']}' aria-hidden='true'></i>
