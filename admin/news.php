  ...
  <body>
	...
	h1 {
		...
		<div class="container" >
			...
			<hr>
				<table class="table-condensed" data-id="news">
			  		<thead>
					    <tr>
					      <th scope="col"><strong>№</strong></th>
					      <th scope="col"><strong>Заголвок</strong></th>
					      <th scope="col"><strong>Текст</strong></th>
					...
					<tbody id="content">
						<?php
		                  $query = "SELECT * FROM news JOIN images ON news.news_image=images.image_id"; //записываем запрос на выборку данных
		                  ...
		                  for($i = 0, $count = sizeof($data); $i < $count; $i++) // выводим данные в виде строк HTML-таблицы 
		                  {
		                  	echo "
					      		...
					      		<td><input class='form-control' name='news_title' disabled type='text' data-type='s' value='{$data[$i]['news_title']}'></td>

					      		<td><input class='form-control' name='news_text' disabled data-type='s' type='text' value='{$data[$i]['news_text']}' ></td>

					      		<td><img src='{$data[$i]['image']}' data-type='s' alt='{$data[$i]['image']}'></td>
					      		<td>
								<div class='action'>
									<i class='fa fa-pencil' aria-hidden='true'></i>
									<i class='fa fa-trash' data-table='news' data-id='{$data[$i]['id']}' aria-hidden='true'></i>
								</div>
								<div class='pencil'>
									<i class='fa fa-check' data-table='news' data-id='{$data[$i]['id']}' aria-hidden='true'></i>
